# ----------------------------------
# version: 0.3 (save annotated video as MP4)
# ----------------------------------
import rclpy
...
class PoseAppNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('output_path', 'annotated_output.mp4')
        ...
        self.timer_period = 1.0 / 30.0  # 30 FPS
        self.t_ms = 0
    ...
    def _init_video_writer(self, width, height, fps=8):
        """Initialize MP4 video writer once."""
        if self.video_writer is not None:
            return  # wait until we have received at least one frame

        os.makedirs(os.path.dirname(self.output_path) or ".", exist_ok=True)
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')  # H.264 / MP4 codec
        self.video_writer = cv2.VideoWriter(self.output_path, fourcc, fps, (width, height))
        if not self.video_writer.isOpened():
            self.get_logger().error("Failed to initialize MP4 writer.")
        else:
            self.get_logger().info(f"MP4 video writer initialized → {self.output_path}")
    ...
    def process_frame(self):
        ...
        for pid, lm_list in enumerate(people):
            ...
            cv2.putText(annotated, f"{pid}", (x0, max(20, y0 - 10)),
    ...
    def destroy_node(self):
        if self.video_writer is not None:
            ...
            self.get_logger().info(f"Saved annotated MP4 video to {self.output_path}")
